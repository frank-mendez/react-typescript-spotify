...
export function LibraryListItem({ item, onNavigate, onPlay }: Readonly<LibraryListItemProps>) {
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      className="group flex items-center gap-3 px-2 py-2 rounded hover:bg-white/10 transition-colors text-left w-full cursor-pointer"
    >
      <div className={`relative shrink-0 w-12 h-12 ${item.isArtist ? 'rounded-full' : 'rounded'} overflow-hidden bg-[#2a2a2a]`}>
        ...
        <PlayButtonOverlay onPlay={(e) => { e.stopPropagation(); onPlay(item, e); }} />
      </div>
